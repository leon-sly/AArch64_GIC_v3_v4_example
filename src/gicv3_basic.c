...
// is provided 揳s is? without warranty of any kind, express or implied. In no
...
uint32_t enableInt(uint32_t ID, uint32_t rd)
{
  ...
  else if (ID < 1020)
  {
    // SPI
    // 中文说明：
    // SPI 的使能位不是“一条中断一个寄存器”，而是 32 个中断共用一个 32 位寄存器。
    // 所以这里要先做两件事：
    // 1. 算出应该访问 GICD_ISENABLER[] 的哪一个寄存器
    // 2. 算出该寄存器里的哪一位对应当前 INTID
    //
    // 例如：
    // - INTID 34 -> bank = 34/32 = 1，说明它落在第 2 个 ISENABLER 寄存器
    // - 34 & 0x1f = 2，说明它对应这个寄存器里的 bit2
    //
    // 最后写入的不是“34”这个数字，而是一个位掩码：
    // 1 << 2 -> 0x00000004
    // 这样硬件就知道要置位的是该寄存器的第 2 位，也就是使能 INTID 34。
...
uint32_t setIntPriority(uint32_t ID, uint32_t rd, uint8_t priority)
{
  ...
  // 中文说明：
  // 这个函数表面上是在“设置中断优先级”，但内部不能只写一个固定寄存器，
  // 因为不同编号范围的中断，落在 GIC 的不同寄存器区。
  //
  // 大致分四类：
  // - 普通 SGI/PPI        -> Redistributor 的 GICR_IPRIORITYR[]
  // - 普通 SPI            -> Distributor 的 GICD_IPRIORITYR[]
  // - 扩展 PPI (1056..1119) -> 仍属本地中断，但编号已进入 GICv3.1 扩展范围
  // - 扩展 SPI (4096..5119) -> 使用 Distributor 的扩展优先级寄存器
  //
  // 所以 ((ID > 1055) && (ID < 1120)) 不是“多余的特殊处理”，
  // 而是在判断：这个 INTID 属于 GICv3.1 Extended PPI，需要换一套索引方式。
  ...
  {
    // Extended PPI
    // 这里之所以不能继续按普通 PPI 的 ID 直接写，
    // 是因为扩展 PPI 的架构编号和寄存器数组下标不是同一个值。
    // 需要先确认硬件确实实现了 ExtPPI，再把架构 ID 转成扩展区里的相对偏移。
    ...
    // 把架构层 INTID 转成扩展 PPI 寄存器区的相对索引。
    // 例如 1056 -> 32，这样才能正确落到 GICR_IPRIORITYR[] 对应位置。
    ID   = ID - 1024;
    gic_rdist[rd].sgis.GICR_IPRIORITYR[ID] = priority;
  }
  else if ((ID > 4095) && (ID < 5120))
